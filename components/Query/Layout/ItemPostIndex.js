...
const IteamPost = memo((props) => {
    const [stateImg, setStateImg] = useState({
        url: props.item.image_thumbnail.data.thumbnails[6].url,
        alt: props.item.image_thumbnail.title,
    });
    const [stateUser, setStateUser] = useState({
        urlAv: props.item.userid.avatar.data.thumbnails[1].url,
        altAv: props.item.userid.name,
        nameAV: props.item.userid.name,
    });
                ...
                <div className="xs:px-3 md:px-6">
                    <div className={` w-full flex items-center justify-between`}>
                        <div className="flex items-center">
                            <div className="mr-5 xs:w-10 md:w-8 xs:h-10 md:h-8" ><img className="rounded-full xs:w-10 md:w-8 xs:h-10 md:h-8" src={stateUser.urlAv} alt={stateUser.altAv} /></div>
                        </div>
